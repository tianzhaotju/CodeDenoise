...
java_special_ids = ["main", "args", "Math", "System", "Random", "Byte", "Short", "Integer", "Long", "Float", "Double",
                    ...
                    "HashMap",
                    'Throwable', 'Class', 'InputStream', 'OutputStream', 'URL', 'HttpServletRequest', 'PrintWriter',
                    'HttpServletResponse',
                    'Thread', 'File', 'IOUtils', 'DataType', 'Writer', 'URLConnection', 'FileOutputStream', 'Browser',
                    'Connection',
...
c_special_ids = ["main",
                 ...
                 "sin", "cos", "tan", "asin", "acos", "atan", "atan2", "sinh", "cosh", "tanh", "exp", "sqrt", "log",
                 "log10", "pow", "powf", "ceil", "floor", "abs", "fabs", "cabs", "frexp", "ldexp", "modf", "fmod",
                 "hypot", "ldexp", "poly", "matherr"]
special_char = ['[', ']', ':', ',', '.', '(', ')', '{', '}', 'not', 'is', '=', "+=", '-=', "<", ">", '+', '-', '*', '/',
                '|']
...
def get_example_codet5(code, tgt_word, lang):
    parser = parsers[lang]
    code = code.replace("\\n", "\n")
    parser = parsers[lang]
    tree = parser[0].parse(bytes(code, 'utf8'))
    ...
    for line in replace_pos.keys():
        for index, pos in enumerate(replace_pos[line]):
            substitute = '<extra_id_' + str(pos_number) + '>'
            diff = len(substitute) - len(tgt_word)
            code[line] = code[line][:pos[0] + index * diff] + substitute + code[line][pos[1] + index * diff:]
            pos_number += 1
    return "\n".join(code)
    ...
    if random.random() <= code_style[4][1]:
        if lang in ['c', 'java']:
            ...
            while i < len(source_code_list):
                ...
                if line.find('for') != -1 and len(line.split('for')) > 1 and len(line.split('for')[1].strip()) > 0 and \
                        line.split('for')[1].strip()[0] == '(':
                    try:
    ...
    if random.random() <= code_style[0][0]:
        if lang == 'c':
            ...
            while source_code.find('printf("', end) != -1:
                ...
                source_code = pre_code + 'char ' + all_variable_name[0] + '[' + str(
                    len(string)) + '] = ' + string + ';' + \
        ...
        if lang in ['c', 'java']:
            ...
            for unary_operator in all_unary_operator:
                ...
                while source_code.find(unary_operator, index) != -1:
                    ...
                    temp_var = \
                    source_code[:index].strip().strip().split('\n')[-1].split(';')[-1].split('(')[-1].split(' ')[-1]
